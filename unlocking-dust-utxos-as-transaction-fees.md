...
While the current threshold of 546 satoshis is a stable starting point, future updates to this BIP could explore implementing a dynamic dust threshold that adjusts based on prevailing median transaction fees (e.g., recalculated every 2016 blocks). A dynamic threshold would ensure the dust definition remains economically relevant as Bitcoin's value and network fees evolve over time, preventing future accumulation of newly uneconomical UTXOs. However, adopting a dynamic threshold introduces additional complexity and potential consensus risks, so it warrants careful consideration and thorough testing before deployment. This possibility is intentionally deferred to future enhancements to keep the initial proposal straightforward, conservative, and consensus-safe.
